<#
.SYNOPSIS
    Gets the Dell BIOS power profile for one or more hosts in vCenter.
...
.DESCRIPTION
    ...
    for the power profile setting. Returns the resulting object as standard output.
...
.EXAMPLE
    .\Get-DellPowerProfile.ps1 myhost.domain.local

    Gets the power profile for a single host.

.EXAMPLE
    .\Get-DellPowerProfile.ps1 (Get-Cluster MyCluster | Get-VMHost)

    Gets the power profile for all hosts in the cluster.
