<#
.SYNOPSIS
    Update the iDRAC firmware for one or more Dell hosts in vCenter.

.DESCRIPTION
    Gets one or more Dell hosts from vCenter, gets the iDRAC IP address from the
    hardware using WSMAN, and attempts to login to each iDRAC and update the
	iDRAC firmware.
...
.PARAMETER Uri
    Path to the iDRAC firmware update file (Windows DUP format).  FTP, HTTP, NFS,
	CIFS, and TFTP are supported.

.EXAMPLE
    .\Update-iDracFirmware.ps1 myhost.domain.local

    Updates the iDRAC firmware for a single host.

.EXAMPLE
    .\Update-iDracFirmware.ps1 (Get-Cluster MyCluster| Get-VMHost)

    Updates the iDRAC firmware for all hosts in the cluster.
#>
#Requires -version 3
...
ForEach ($vmhost in $vmhosts) {
    ...
    # Get iDRAC CIM instance
	$resourceUri="http://schemas.dell.com/wbem/wscim/1/cim-schema/2/DCIM_SoftwareIdentity"
	$properties = @{InstanceID="DCIM:INSTALLED#iDRAC.Embedded.1-1#IDRACinfo"}
	...
	$jobid = $responseData.Job.EndpointReference | select InstanceID
