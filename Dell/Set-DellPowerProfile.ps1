<#
.SYNOPSIS
    Sets the Dell BIOS power profile for one or more hosts in vCenter.
...
.DESCRIPTION
    ...
    power profile setting.  Schedules a job through the iDRAC to commit the
...
.EXAMPLE
    .\Set-DellPowerProfile.ps1 myhost.domain.local

    Sets the power profile for a single host.

.EXAMPLE
    .\Set-DellPowerProfile.ps1 (Get-Cluster MyCluster| Get-VMHost)

    Sets the power profile for all hosts in the cluster.
.NOTES
    This performance study is now a bit dated, but still a good reference.
...
#Requires -modules VMware.VimAutomation.Core
 param (
    ...
    $DracPassword,
    $OutputFile,
    $PowerProfile = 'PerfPerWattOptimizedOs'
 )
